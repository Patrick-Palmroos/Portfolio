...
export default function FilterButton({ title, active = false, callback }: { title: string; active?: boolean; callback: () => void }) {
    ...
    <div>
      <Box
        ...
        sx={{
          cursor: 'pointer',
          position: 'relative',
          width: isMobile ? '11rem' : isVeryTiny ? '11rem' : '9rem',
          ...
          '&::before': {
            ...
            borderWidth: '0.2rem',
            background: 'linear-gradient(#1e1d28, #1e1d28) padding-box, linear-gradient(to right, #FF74D0, #B985FC) border-box',
